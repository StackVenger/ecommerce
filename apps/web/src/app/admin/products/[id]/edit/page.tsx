...
function hydrateVariants(
  raw: unknown,
  rawAttributes?: any[],
): { options: OptionType[]; variants: Variant[] } {
...
export default function AdminProductEditPage() {
  ...
  const fingerprintOptions = (opts: Record<string, string>): string =>
    JSON.stringify(Object.fromEntries(Object.entries(opts).sort(([a], [b]) => a.localeCompare(b))));
    ...
    async function loadProduct() {
      try {
        ...
        initialStockRef.current = {
          ...
          variants: new Map(variants.map((v) => [fingerprintOptions(v.options), v.stock])),
        };
  ...
  const refreshStockFromServer = useCallback(async () => {
    ...
    try {
      ...
      setFormData((prev) => {
        ...
        if (
          prev.quantity === initialStockRef.current.productQuantity &&
          liveQuantity !== prev.quantity
        ) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const productQuantityDirty = formData.quantity !== initialStockRef.current.productQuantity;
      ...
      initialStockRef.current = {
        ...
        variants: new Map(hydrated.variants.map((v) => [fingerprintOptions(v.options), v.stock])),
      };
      setFormData((prev) => ({
        ...prev,
              ...
              <label className="field-label">
                ...
                {formData.description.trim().length > 0 &&
                  formData.descriptionBn.trim().length === 0 && (
                    <span className="pill pill-warning ml-2 px-2 py-0.5">Bn missing</span>
                  )}
              </label>
              <RichTextEditor
