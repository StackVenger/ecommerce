...
type GetConstructorType<T extends AnyConstructor> = T extends new (...args: any) => infer C // deno-lint-ignore no-explicit-any
  ? C
  : never;
 ...
 * ```
 */
export function assertArrayIncludes<T>(actual: ArrayLike<T>, expected: ArrayLike<T>, msg?: string) {
...
export function assertObjectMatch(
  ...
  function filter(a: loose, b: loose) {
    ...
    function fn(a: loose, b: loose): loose {
      ...
      const entries = [...Object.getOwnPropertyNames(a), ...Object.getOwnPropertySymbols(a)]
        .filter((key) => key in b)
            ...
            if (value instanceof Map && subset instanceof Map) {
              filtered[key] = new Map(
                [...value]
                  ...
                  .map(([k, v]) => [k, typeof v === 'object' ? fn(v, subset.get(k)) : v]),
              );
              continue;
            }
...
export function assertIsError<E extends Error = Error>(
  ...
  if (
    msgIncludes &&
    (!(error instanceof Error) || !stripColor(error.message).includes(stripColor(msgIncludes)))
  ) {
