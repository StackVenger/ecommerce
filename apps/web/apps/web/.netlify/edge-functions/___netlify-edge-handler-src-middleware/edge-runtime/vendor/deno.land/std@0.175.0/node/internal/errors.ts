 ...
 * @return The error.
 */
export const uvExceptionWithHostPort = hideStackFrames(function uvExceptionWithHostPort(
  err: number,
  syscall: string,
  address?: string | null,
  port?: number | null,
 ...
 * @param additional
 */
export const exceptionWithHostPort = hideStackFrames(function exceptionWithHostPort(
  err: number,
  syscall: string,
  address: string,
  port: number,
...
export class ERR_ENCODING_INVALID_ENCODED_DATA extends NodeErrorAbstraction implements TypeError {
  errno: number;
...
export class ERR_INVALID_PACKAGE_TARGET extends NodeError {
  constructor(
    ...
    let msg: string;
    const relError =
      typeof target === 'string' && !isImport && target.length && !target.startsWith('./');
    if (key === '.') {
...
export class ERR_PACKAGE_IMPORT_NOT_DEFINED extends NodeTypeError {
  constructor(specifier: string, packagePath: string | undefined, base: string) {
