...
function innerDeepEqual(val1: unknown, val2: unknown, strict: boolean, memos = memo): boolean {
  ...
  if (Array.isArray(val1)) {
    ...
    return keyCheck(val1 as object, val2 as object, strict, memos, valueType.noIterator);
    ...
    return keyCheck(val1 as object, val2 as object, strict, memos, valueType.noIterator, keysVal1);
  } else if (isSet(val1)) {
    if (!isSet(val2) || (val1 as Set<unknown>).size !== (val2 as Set<unknown>).size) {
      return false;
    }
    return keyCheck(val1 as object, val2 as object, strict, memos, valueType.isSet);
  } else if (isMap(val1)) {
    if (!isMap(val2) || (val1 as Set<unknown>).size !== (val2 as Set<unknown>).size) {
      return false;
    }
    return keyCheck(val1 as object, val2 as object, strict, memos, valueType.isMap);
...
function isEqualBoxedPrimitive(a: any, b: any): boolean {
  ...
  if (isStringObject(a)) {
    return (
      isStringObject(b) && String.prototype.valueOf.call(a) === String.prototype.valueOf.call(b)
    );
  }
  if (isBooleanObject(a)) {
    return (
      isBooleanObject(b) && Boolean.prototype.valueOf.call(a) === Boolean.prototype.valueOf.call(b)
    );
  }
  if (isBigIntObject(a)) {
    return (
      isBigIntObject(b) && BigInt.prototype.valueOf.call(a) === BigInt.prototype.valueOf.call(b)
    );
  }
  if (isSymbolObject(a)) {
    return (
      isSymbolObject(b) && Symbol.prototype.valueOf.call(a) === Symbol.prototype.valueOf.call(b)
    );
  }
...
function findLooseMatchingPrimitives(primitive: unknown): boolean | null | undefined {
...
function setMightHaveLoosePrim(set1: Set<unknown>, set2: Set<unknown>, primitive: any) {
...
function setHasEqualElement(set: any, val1: any, strict: boolean, memos: Memo): boolean {
