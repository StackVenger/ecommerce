...
import { AssertionError, AssertionErrorConstructorOptions } from './assertion_error.ts';
...
function createAssertionError(options: ExtendedAssertionErrorConstructorOptions): AssertionError {
...
function throws(fn: () => void, error?: RegExp | Function | Error, message?: string) {
  // Check arg types
  ...
  if (typeof message === 'string') {
    if (
      ...
      throw new ERR_INVALID_ARG_TYPE('error', ['Function', 'Error', 'RegExp', 'Object'], error);
    }
  } else {
    if (
...
function doesNotThrow(fn: () => void, error?: Function, message?: string | Error): void;
function doesNotThrow(fn: () => void, error?: RegExp, message?: string): void;
...
function equal(actual: unknown, expected: unknown, message?: string | Error) {
...
function notEqual(actual: unknown, expected: unknown, message?: string | Error) {
  if (arguments.length < 2) {
    throw new ERR_MISSING_ARGS('actual', 'expected');
  }
...
function strictEqual(actual: unknown, expected: unknown, message?: string | Error) {
  if (arguments.length < 2) {
    throw new ERR_MISSING_ARGS('actual', 'expected');
  }
...
function notStrictEqual(actual: unknown, expected: unknown, message?: string | Error) {
  if (arguments.length < 2) {
    throw new ERR_MISSING_ARGS('actual', 'expected');
  }
...
function deepEqual(actual: unknown, expected: unknown, message?: string | Error) {
  if (arguments.length < 2) {
    throw new ERR_MISSING_ARGS('actual', 'expected');
  }
...
function notDeepEqual(actual: unknown, expected: unknown, message?: string | Error) {
  if (arguments.length < 2) {
    throw new ERR_MISSING_ARGS('actual', 'expected');
  }
...
function deepStrictEqual(actual: unknown, expected: unknown, message?: string | Error) {
  if (arguments.length < 2) {
    throw new ERR_MISSING_ARGS('actual', 'expected');
  }
...
function doesNotMatch(string: string, regexp: RegExp, message?: string | Error) {
  if (arguments.length < 2) {
...
function rejects(
  ...
  if (typeof asyncFn === 'function') {
    ...
    if (!isValidThenable(promise)) {
      return Promise.reject(
        new ERR_INVALID_RETURN_VALUE('instance of Promise', 'promiseFn', promise),
      );
    }
  ...
  function rejects_onRejected(e: Error) {
    // TODO(uki00a): In order to `test-assert-async.js` pass, intentionally adds `rejects_` as a prefix.
    if (
      validateThrownError(e, error, message, {
        operator: rejects,
  ...
  if (typeof asyncFn === 'function') {
    try {
      ...
      if (!isValidThenable(value)) {
        return Promise.reject(
          new ERR_INVALID_RETURN_VALUE('instance of Promise', 'promiseFn', value),
        );
      }
      promise = value;
    } catch (e) {
...
function isValidThenable(maybeThennable: any): boolean {
  ...
  const isThenable =
    typeof maybeThennable.then === 'function' && typeof maybeThennable.catch === 'function';
